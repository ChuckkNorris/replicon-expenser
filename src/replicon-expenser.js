...
const loginToReplicon = async (page, email, password) => {
  ...
  console.log('Waiting to select organization AD type...');
  await page.clickAndWaitSelector('[type=submit]', '#aadTile')
  ...
  if (staySignedInText && staySignedInText.indexOf("times you are asked to sign in.") >= 0) {
...
const clickDetailButton = async (page, rowId) => {
  ...
  const duplicateLink = await page.$(`a[onclick="expGUI.duplicateExpenseLine('${rowId}')"`);
  const detailButton = await page.evaluateHandle(el => el.previousElementSibling.previousElementSibling, duplicateLink);
...
const getReceipts = async (directoryPath) => {
  ...
  for (fileName of files) {
    ...
    receipts.push({
      ...
      purpose: split[2],
      place: split[3],
      amount: (amount[0] === '$' ? amount.substring(1) : amount).split('.')[0]
    });
  }
  return receipts;
}
...
const createNewExpenseReport = async (page, receipts) => {
  ...
  await page.clickAndWaitSelector('a[href="/Credera/my/expenses/"]', '#addNewExpenseButton');
